...
class Presence {
    ...
    public function ajouterPresence($idUtilisateur, $date, $arrive, $depart, $idLieu) {
        $stmt = $this->db->prepare("INSERT INTO Presence (date, arrive, depart, actif, idUtilisateur, idLieu)
                                    VALUES (?, ?, ?, 1, ?, ?)");
        return $stmt->execute([$date, $arrive, $depart, $idUtilisateur, $idLieu]);
    }
    ...
    public function getVendeursActifs() {
        ...
        $stmt = $this->db->query("SELECT Presence.*, Vendeur.nomFoodTruck, Utilisateur.nom, Utilisateur.prenom,
                                Lieu.cp, Lieu.ville, Lieu.rue, Lieu.coordLat, Lieu.coordLong
                                FROM Presence
                                JOIN Vendeur ON Presence.idUtilisateur = Vendeur.idUtilisateur
                                JOIN Utilisateur ON Vendeur.idUtilisateur = Utilisateur.idUtilisateur
                                JOIN Lieu ON Presence.idLieu = Lieu.idLieu
                                WHERE Presence.date = CURDATE() AND Presence.arrive <= CURTIME()
